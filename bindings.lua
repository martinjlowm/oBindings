...
local bindKey = function(key, action, mod)
    ...
    if(not action) then
        ...
        btn:SetAttribute(string.format('ob-%s-attribute', mod or 'base'), ty .. ',' .. action:gsub('\n', ''):trim())
...
function _NS:PLAYER_TALENT_UPDATE()
    local activeSpecialization = GetSpecialization(nil, false)
    local numSpecializations = GetNumSpecializations(false, false)

    if (numSpecializations == 0) then
        return
    end

    local _, name = GetSpecializationInfo(activeSpecialization)
